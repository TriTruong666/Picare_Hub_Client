...
import type { Frame, FramePass } from 'vgpu';
    ...
    void (async () => {
      try {
        ...
        const renderFrame = (currentFrame: Frame) => {
          ...
          if (!postEnabled) {
            currentFrame.pass({ target: output, clear: settings.background }, (pass: FramePass) => {
              pass.draw(graph.shardDraw, { instances: activeCount });
            });
          } else {
            currentFrame.pass({ target: graph.sceneTarget, clear: settings.background }, (pass: FramePass) => {
              pass.draw(graph.shardDraw, { instances: activeCount });
            });
            if (settings.effect === EFFECTS.ascii) {
              currentFrame.pass({ target: graph.asciiTarget, clear: [0, 0, 0, 0] }, (pass: FramePass) => {
                pass.draw(graph.asciiEffect);
              });
            }
            if (settings.effect !== EFFECTS.none) {
              currentFrame.pass({ target: graph.styleTarget, clear: settings.background }, (pass: FramePass) => {
                pass.draw(graph.styleEffect);
              });
            }
            if (settings.bloom > 0.0001) {
              currentFrame.pass({ target: graph.bloomTarget, clear: [0, 0, 0, 1] }, (pass: FramePass) => {
                pass.draw(graph.bloomEffect);
              });
              currentFrame.pass({ target: graph.bloomScratchTarget, clear: [0, 0, 0, 1] }, (pass: FramePass) => {
